...
class PathAnimation(Scene):
    def __init__(
        ...
        super().__init__(
            renderer, camera_class, always_update_mobjects, random_seed, skip_animations
        )
    ...
    def construct(self):
        ...
        for curr_round in range(self.rounds_per_temp):
            paths.append(
                data[self.model_name][f"TEMP_{temperature}"][f"R_{curr_round}"][1]
            )
        ...
        step_text = Text(
            f"Step {step_number}", font_size=18, color=WHITE, font="Cascadia Mono"
        ).to_corner(DL)
    ...
    def add_legend(self, colors, num_rounds):
        ...
        for i in range(num_rounds):
            color = colors[i % len(colors)]
            square = Square(
                side_length=0.3, fill_color=color, fill_opacity=1
            ).set_stroke(width=0)
            label = Text(
                f"Round {i+1}", font_size=15, color=WHITE, font="Cascadia Mono"
            ).next_to(square, RIGHT, buff=0.2)
