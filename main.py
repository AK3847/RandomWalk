    ...
    for curr_temp in TEMP_RANGE:
        ...
        for curr_round in range(ROUNDS_PER_TEMP):
            ...
            for step in range(TOTAL_STEPS):
                data[f"{MODEL_NAME}"][f"TEMP_{curr_temp}"][f"R_{curr_round}"][1].append(current_position.copy())
                ...
                data[f"{MODEL_NAME}"][f"TEMP_{curr_temp}"][f"R_{curr_round}"][0].append(reply)


                path.append(current_position.copy())
        end_time = time.time() - start_time
        print(f"--- {ROUNDS_PER_TEMP} rounds done for temperature - {curr_temp} - time taken : {end_time:.2f} seconds")
...
if __name__ == "__main__":
    ...
    modified_file_name = str(MODEL_NAME).split(":")[0]+"_"+str(MODEL_NAME).split(":")[1]
